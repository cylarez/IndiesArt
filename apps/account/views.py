...
def submissionNew(request):
    ...
    if request.method == 'POST' and len(request.FILES.getlist('file_upload')) > 0:
        ...
        captcha_response = \
        captcha.submit(request.POST.get("recaptcha_challenge_field", None), request.POST.get("recaptcha_response_field", None), '6LeFMskSAAAAAJtFwRe1B4H1X817AHGZE8z66mLD', request.META.get("REMOTE_ADDR", None))
    ...
    return render_to_response('account/submission-new.html', {'category': category, 'urlType': urlType, 'captcha_error': captcha_error, 'posted': request.method == 'POST' and captcha_error == ""}, context_instance=RequestContext(request))
...
def submissionList(request):
	...
	return render_to_response('account/submission-list.html', {'images':images}, context_instance=RequestContext(request))